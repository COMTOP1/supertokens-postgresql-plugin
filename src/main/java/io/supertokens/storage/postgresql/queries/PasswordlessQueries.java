...
public class PasswordlessQueries {
    ...
    /*public static List<UserInfo> getUsersByIdList(Start start, Set<String> ids,
                                                  AppIdentifier appIdentifier)
            throws SQLException, StorageQueryException {
        if (ids.size() > 0) {
            // No need to filter based on tenantId because the id list is already filtered for a tenant
    ...
    public static UserInfo getUserByEmail(Start start, AppIdentifier appIdentifier, @Nonnull String email)
            throws StorageQueryException, SQLException {
        String QUERY = "SELECT user_id, email, phone_number, time_joined FROM "
                + getConfig(start).getPasswordlessUsersTable() + " WHERE app_id = ? AND email = ?";
